...
echo "Building tflite MT for wasm..."
./bazel build --config=wasm_mt -c opt //tensorflow/lite/core/c:c_api //tensorflow/lite/delegates/xnnpack:xnnpack_delegate
emar qsL libtflite_c-mt.a `find bazel-bin/ -name '*.o'`
echo "Building a monolithic object to check for duplicate symbols..."
emcc -o libtflite_c-mt.so -r libtflite_c-mt.a -s LINKABLE
